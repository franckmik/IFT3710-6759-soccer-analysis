...
def train_model(model,batch_size,num_epochs, learning_rate,device,
                train_loader,val_loader,saved_t=None, saved_v=None,saved_acc=None):
  """
  ...
  if saved_t is not None:
    train_loss_history=saved_t
    val_loss_history= saved_v
    acc_history= saved_acc
    n= len(saved_t)
    epochs= num_epochs-n
  else:
    train_loss_history=[]
  ...
  for epoch in range(epochs,num_epochs+1):
    ...
    #accuracy plot
      plt.subplot(1,2,2)
      plt.plot(acc_history,label='Val acc', color='green')
    ...
    #progress logging
      ...
      print(f'{epoch+1 if len(saved_t) ==0 else epoch}/{num_epochs}|' 
    ...
    #dynamic early stopping
      if (epoch >20) and (np.mean(acc_history[-5:])<best_acc):
  ...
  #save model
  torch.save(model.state_dict(),'model_event_classifierV2.pth')
